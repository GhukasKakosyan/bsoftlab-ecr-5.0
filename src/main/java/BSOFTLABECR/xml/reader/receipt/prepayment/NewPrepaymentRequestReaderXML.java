...
public class NewPrepaymentRequestReaderXML {
    ...
    public NewPrepaymentRequest readFile() throws FileNotFoundException, XMLStreamException {
        ...
        while(xmlEventReader.hasNext()) {
            ...
            if(xmlEvent.isEndElement() && xmlEvent.asEndElement().getName().getLocalPart().equals(REQUEST)) {
                if(!((newPrepaymentRequest != null)
                        && (newPrepaymentRequest.getSeq() != -1)
                        && (newPrepaymentRequest.getPaidAmount() != -1)
                        && (newPrepaymentRequest.getPaidAmountCard() != -1)
                        && (newPrepaymentRequest.getPartialAmount() != -1)
                        && (newPrepaymentRequest.getPrePaymentAmount() != -1)
                        && (newPrepaymentRequest.getMode() != -1)
                        && (newPrepaymentRequest.isUseExtPOS())
                        && (newPrepaymentRequest.getItems() == null)))
                    newPrepaymentRequest = null;
                break;
            }
        }
